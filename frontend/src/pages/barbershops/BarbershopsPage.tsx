...
import { Plus, Search, MapPin, Phone, Mail, Users, Scissors, Calendar } from 'lucide-react';
...
export const BarbershopsPage: React.FC = () => {
  ...
  const canCreateBarbershop = user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN';
  ...
  const fetchBarbershops = async (page = 1, search = '') => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Erro ao carregar barbearias');
    } finally {
      setLoading(false);
    }
  };
      ...
      {barbershops.length === 0 && !loading ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            {searchTerm ? 'Tente ajustar os termos de busca.' : 'Comece criando sua primeira barbearia.'}
          </p>
              ...
              <div className="p-6">
                ...
                <div className="border-t pt-3">
                  <p className="text-xs text-gray-500">
                    Proprietário: <span className="font-medium">{barbershop.owner.name}</span>
                  </p>
                </div>
                ...
                <div className="flex gap-2 mt-4">
                  ...
                  {(user?.role === 'SUPER_ADMIN' || barbershop.owner.id === user?.id) && (
                    <Link
