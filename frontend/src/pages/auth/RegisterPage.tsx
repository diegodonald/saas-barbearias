...
const RegisterPage: React.FC = () => {
  ...
  const { register, handleSubmit, getFieldError, isSubmitting, watch } =
    useRegisterForm();
      ...
      <div className='max-w-md w-full space-y-8'>
                ...
                <div className='mt-2 space-y-1'>
                  ...
                  {passwordRequirements.map((req, index) => (
                    <div key={index} className='flex items-center gap-2'>
                      ...
                      />
                      <span
                        className={`text-xs ${req.test(password) ? 'text-success-600' : 'text-secondary-500'}`}
                      >
                        {req.label}
                      </span>
                    </div>
        ...
        <div className='text-center'>
          <p className='text-xs text-secondary-500'>
            Ao criar uma conta, você concorda em receber comunicações sobre
            nossos serviços. Você pode cancelar a qualquer momento.
          </p>
        </div>
      </div>
    </div>
